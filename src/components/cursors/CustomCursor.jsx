...
export default function Cursor() {
    ...
    useEffect(() => {
        if (!isHolding && wasHolding) {
            const timeout = setTimeout(() => setWasHolding(false), 400)
        ...
        <>
            <div
                ...
                className={`custom-cursor large
                    ${isLargeClick && !isHolding ? 'click' : ''}
                    ${isHolding ? 'hold' : ''}
                    ${isHovering ? 'fade-out' : 'fade-in'}
                    ${isCursorVisible ? '' : 'transparent'}`
                }
                style={{
            ...
            <div
                ...
                className={`custom-cursor small
                    ${isSmallClick ? 'click' : ''}
                    ${isHovering && !isHolding ? 'enlarge' : ''}
                    ${isHolding ? 'hold' : ''}
                    ${!isHolding && wasHolding ? 'hold-exit' : ''}
                    ${isCursorVisible ? '' : 'transparent'}`
                }
            />
        </>
